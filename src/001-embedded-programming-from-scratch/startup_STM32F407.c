 ...
 *                   Implements the reset handler that performs operations that
 *                   needs to be done before invoking main()
 */
...
void Reset_Handler(void) {
    /* Copy the .data section from Flash TO SRAM
            ...
            - Linker script creates entries in the symbol table of final elf
       file
    */
    uint32_t size_data =
        (uint32_t)&_edata - (uint32_t)&_sdata; // Size of the data section
